...
def main():
    ...
    # Check for unique gene tags
    duplicate_gene_tags = cds_db.select(pl.col('gene_tag')).is_duplicated()
    if duplicate_gene_tags.any():
        doubles = cds_db.filter(duplicate_gene_tags).select(pl.col('gene_tag')).to_series().unique().to_list()
        # print(cds_db.filter(duplicate_gene_tags).sort('gene_tag'))
        LOG.critical(f'Duplicate gene tags detected! {" ".join(doubles)}')
        sys.exit()
    ...
    if multi_taxa_contig.any():
        LOG.warning("Detected duplicate contig labels! Trying to fix this by prepending the locus tag prefix...")
        ...
        if multi_taxa_contig_fixed.any():
            LOG.critical('Still detected duplicate contig labels! Exiting.')
            sys.exit()
        else:
            LOG.warning('Fix succeeded!')
